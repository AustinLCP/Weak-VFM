...
def calc_dis_ray_tracing(wl, Ry, points, density, bev_box_center):
    ...
    # print("0: " + str(((dis_all[dis_in_mul].view(-1, 2))[z_buffer_ind_gather]).shape))
    # print("3: " + str(density.shape))
...
def build_loss(pred_3D, batch_RoI_points, batch_lidar_y_center,
                             ...
                             bbox2d, batch_dim):
    all_loss, count = 0, 0
    per_sample = batch_RoI_points.shape[1]  # 3个bbox对应的RoI points
    fx = P2[:, 0, 0].view(-1, 1).repeat(1, per_sample).view(-1)  # focal length(焦距) fx
    fy = P2[:, 1, 1].view(-1, 1).repeat(1, per_sample).view(-1)  # focal length(焦距) fy
    cx = P2[:, 0, 2].view(-1, 1).repeat(1, per_sample).view(-1)  # principal point cx 相机光学中心，主点
    cy = P2[:, 1, 2].view(-1, 1).repeat(1, per_sample).view(-1)  # principal point cy

    p_locXY, p_locZ, p_ortConf = pred_3D


    bbox2d = bbox2d.view(-1, 4)  # 存的是bbox四个角的坐标
    p_locXY = p_locXY.view(-1, 2)
    p_locZ = p_locZ.view(-1, 1)
    p_ortConf = p_ortConf.view(-1, 2)
    p_locXYZ = torch.cat([p_locXY, p_locZ], dim=1)

    num_instance = bbox2d.shape[0]  # 一个batch共有8x3=24个bbox
    batch_RoI_points = batch_RoI_points.view(num_instance, -1, 3)  # 一个bbox对应100个RoI_points
    batch_lidar_y_center = batch_lidar_y_center.view(num_instance)  # 一个bbox对应一个center的y坐标
    batch_lidar_orient = batch_lidar_orient.view(num_instance)  # 一个bbox对应一个orientation
    batch_lidar_density = batch_lidar_density.view(num_instance, -1)  # 一个bbox对应100个density(float)
    abs_dim = batch_dim.view(num_instance, 3)  # 物体的尺寸(长宽高),初始数据从先验里来

    # ground truth 的 center 坐标
    ...
    #######################################################
    # 考虑物体尺寸及相机参数，校正预测的 x 和 y 坐标 (z 坐标不变) #
    #######################################################
    # 预测框在图像中的中心位置  将预测的x和y坐标通过Sigmoid正则化并中心化    然后缩放到图像的实际尺寸
    proj_box_center = ((F.sigmoid(p_locXYZ[:, :2])-0.5) * torch.stack([w, h], dim=1) + \
                    # 将其转换到以图像中心为原点的坐标系统
                 torch.stack([center_x, center_y], dim=1) - \
                    # 减去光学中心的偏移                     # 通过焦距将这些值转换为以像素为单位的尺寸
                 torch.stack([cx, cy], dim=1)) / torch.stack([fx, fy], dim=1)
    # 将proj_box_center和一个全为1的tensor连接, 相当于给proj_box_center 添加一个全为1的列, 用于简化三维到二维的投影计算
    box_center = torch.cat([proj_box_center, torch.ones((proj_box_center.shape[0], 1)).cuda()], dim=1)
    location_3d = p_locXYZ[:, 2:3] * box_center  # 把p_locXYZ第三列的值(z)赋给box_center全为1的那一列

    #############################################
    # 对对象的三维方向估计，计算对象在世界坐标系中的方位 #
    #############################################
    # 对tensor p_ortConf 进行L2标准化，使得每一行的向量长度为1
    alpha_ratio = F.normalize(p_ortConf, dim=1)
    # 计算这两个值的反正切(arctan), 并把结果限制在[0,pi)内
    Ry_pred = torch.atan2(alpha_ratio[:, 0], alpha_ratio[:, 1]) % np.pi

    Ry = batch_lidar_orient
    Alpha = -batch_lidar_orient
    # .detach() 方法用于停止跟踪此操作的梯度，有助于防止不必要的梯度计算
    trans_Ry = (torch.atan2(location_3d[:, 0], location_3d[:, 2])).detach()
    Alpha = (Alpha - trans_Ry) % np.pi


    for i in range(len(p_locXYZ)):
        single_Ry = Ry[i]  # 单个bbox 的 ground truth 的 orientation
        single_wl = abs_dim[i, 1:]  # 单个 bbox 的 width 和 length
        single_loc = location_3d[i]  # 单个 bbox 的经过相机参数校正的 x,y,z 坐标
        single_Alpha = Alpha[i]  # 单个 bbox 经过处理的 orientation
