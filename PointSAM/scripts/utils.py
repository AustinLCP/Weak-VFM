...
from kitti_lidar_point_cloud import LidarPointCloud
from nuscenes.utils.geometry_utils import transform_matrix, view_points
from pyquaternion import Quaternion
from pykitti.utils import load_velo_scan
...
def load_pointcloud_multisweep_kitti(kitti, sample_rec, sweeps=[0], min_distance=1.0):
    """
    Load a point cloud that aggregates multiple sweeps from KITTI Raw dataset.

    :param kitti: An instance of the KITTI dataset handler.
    :param sample_rec: The current sample/frame.
    :param sweeps: List of sweeps to aggregate, where 0 is the current sample, -1 is the previous sample, 1 is the next sample.
    :param min_distance: Distance below which points are discarded.
    :return: Aggregated point cloud, timestamps, sweep indices, and masks.
    """
    ...
    def get_sample_record_from_idx(ref_sample_data_rec, idx):
        ...
        for _ in range(abs(idx)):
            succeed_token = current_sd_rec['next'] if idx > 0 else current_sd_rec['prev']
            if succeed_token == None:
                break
            else:
                current_sd_rec = kitti.get('sample_data', succeed_token)
        return current_sd_rec
    ...
    ref_sd_token = sample_rec['data']['LIDAR_TOP']
    ref_sd_rec = kitti.get('sample_data', ref_sd_token)

    # Get reference pose and timestamp.
    ref_pose = kitti.get("calibrated_sensor", ref_sd_rec["calibrated_sensor_token"])["ref_pose"]
    ...
    for idx in sweeps:
        ...
        # Load point cloud.
        current_pc = LidarPointCloud.from_file(osp.join(kitti.dataroot, current_sd_rec['filename']))
        current_mask = np.ones(current_pc.nbr_points(), dtype=bool)
        if idx != 0:
            current_mask = remove_close(current_pc.points, min_distance)

        # Get current pose and calculate transformation.
        current_pose = kitti.get("calibrated_sensor", current_sd_rec["calibrated_sensor_token"])["ref_pose"]
        transform = np.dot(np.linalg.inv(ref_pose), current_pose)

        # Apply the transformation to the current point cloud.
        current_pc.points[:3, :] = np.dot(transform[:3, :3], current_pc.points[:3, :]) + transform[:3, 3:4]

        # Compute time lag and sweep index.
        time_lag = ref_time - current_sd_rec["timestamp"]
        times = time_lag * np.ones(current_pc.points.shape[1])
        sweep_idx = idx * np.ones(current_pc.points.shape[1], dtype=np.int32)
...
# def map_proj_to_image_kitti(nusc, point_velo, image, sample_data):
#     calib_sensor = nusc.get('calibrated_sensor', sample_data['calibrated_sensor_token'])
#     R_velo_to_cam = calib_sensor["R_velo_to_cam"]
#     T_velo_to_cam = calib_sensor["T_velo_to_cam"]
#     P_rect = calib_sensor["P_rec_matric"]
#
#     # Step 1: Convert from velodyne coordinates to camera coordinates
#     point_cam = np.dot(R_velo_to_cam, point_velo[:3, :]) + T_velo_to_cam[:, np.newaxis]
#
#     # Step 2: Convert to homogeneous coordinates by adding 1s row
#     point_cam_hom = np.vstack((point_cam, np.ones((1, point_cam.shape[1]))))
#
#     # Step 3: Project onto image plane using P_rect
#     point_img = np.dot(P_rect, point_cam_hom)
#
#     # Step 4: Convert to pixel coordinates by dividing by the third row
#     u = point_img[0, :] / point_img[2, :]
#     v = point_img[1, :] / point_img[2, :]
#
#     # Step 5: Create mask for points that are in front of the camera and within image bounds
#     mask = (point_img[2, :] > 0) & (u > 1) & (u < image.shape[1]) & (v > 1) & (v < image.shape[0])
#
#     # Apply mask to filter points
#     u = u[mask]
#     v = v[mask]
#
#     # Stack and return the 2D coordinates and the mask
#     points_2d = np.vstack((u, v))
#
#     return points_2d, mask

def map_proj_to_image_kitti(l_3d, image, cam_to_cam, velo_to_cam):
    ...
    mask = np.zeros(l_3d.shape[0], dtype=bool)
    mask[valid_ind] = True
