...
def main():
    ...
    # nusc = NuScenes(version='v1.0-trainval', dataroot='data\\nuscenes', verbose=False)
    nusc = KittiLoader()
    segmentor_2d = InstanceSegmentor(nusc, cfg, device="cuda")
    ...
    for sample_idx in local_sample_indices:
        segmentor_2d.save_masks(args.out_dir, sample_idx)  # get semantic mask and instance mask (.npz file for each mask)
        pbar.update(1)
